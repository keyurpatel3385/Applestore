...
}}var c={global:"http://www.apple.com"+d+"/search/",downloads:"http://www.apple.com"+d+"/search/downloads/",iphone:"http://www.apple.com"+d+"/search/iphone/",ipoditunes:"http://www.apple.com"+d+"/search/ipoditunes/",mac:"http://www.apple.com"+d+"/search/mac/",store:"http://www.apple.com"+d+"/search/store/",support:"http://www.apple.com"+d+"/search/support/"};
var b=c[searchSection]||"http://www.apple.com/search/";enhanceSearch(b);var a=navigator.userAgent.match(/AppleWebKit/i)&&navigator.userAgent.match(/Mobile/i);
...
var i=Math.min(F,D);return G>w&&i>B}var a=AC.Quicktime.controllers;var o=$("sp-results");
...
}this.selected=a;a.className="hoverli"},unhighlight:function(a){a.className=""},load:function(){var a=document.createElement("img");
if(location.protocol.match(/https/)){a.src="https://ssl.apple.com/global/nav/images/spinner.gif"
}else{a.src="http://images.apple.com/global/nav/images/spinner.gif"}a.width="11";
a.height="11";a.border="0";a.alt="*";a.id="sp-search-spinner";a.style.display="none";
document.getElementById("globalsearch").appendChild(a);document.getElementById("g-search").onsubmit=function(b){return false
};if(navigator.userAgent.match(/AppleWebKit/i)){document.getElementById("sp-searchtext").onkeydown=function(b){var c=typeof(event)!="undefined"?event.keyCode:b.keyCode;
if(!b){b=event}if(c==13&&!b.altKey){if(b.target.value.length===0){return false}if(SearchShortcut.selected){SearchShortcut.go(SearchShortcut.selected.url)
}else{SearchShortcut.hideResults();document.getElementById("g-search").submit()
}}}}document.getElementById("sp-searchtext").onkeyup=function(b){var d=typeof(event)!="undefined"?event.keyCode:b.keyCode;
if(!b){b=event}if(d==40&&SearchShortcut.results){try{b.preventDefault();b.stopPropagation()
}catch(e){}if(SearchShortcut.selected&&(SearchShortcut.results.length>SearchShortcut.selected.num+1)){SearchShortcut.keyHighlight(document.getElementById("sp-result-"+(SearchShortcut.selected.num+1)))
...
}SearchShortcut.flashDomRender()}else{if(d==38&&SearchShortcut.results){try{b.preventDefault();
b.stopPropagation()}catch(e){}if(SearchShortcut.selected&&SearchShortcut.selected.num>0){SearchShortcut.keyHighlight(document.getElementById("sp-result-"+(SearchShortcut.selected.num-1)))
}SearchShortcut.flashDomRender()}else{if(d==27){SearchShortcut.hideResults();document.getElementById("sp-searchtext").value=""
}else{SearchShortcut.selected=false;var c=document.getElementById("sp-searchtext").value;
c=c.replace(/[%\^\?\!\*\/<>\$]/ig,"");c=c.replace(/^\s+/g,"").replace(/\s+$/g,"");
if(c.length<1&&SearchShortcut.html){SearchShortcut.html.results.innerHTML="";document.getElementById("sp-search-spinner").style.display="none";
SearchShortcut.hideResults()}else{if(c.length>SearchShortcut.minimumCharactersForSearch){SearchShortcut.searchText=c;
